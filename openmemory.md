...
- **`STATUS.json`** / **`STATUS.md`** — PASS 10A sole authoritative current-status surface; `STATUS.md` is rendered only via **`scripts/render_status_surface.py`** (aggregate from inventory, `record.json`, ledger tail, git HEAD, workflow sha, cross-repo check). **PASS 11:** `invariant_summary` (counts by severity + violations from latest run). Policy: `verification/closed-epistemics-open-interfaces-policy.json`. INVARIANT_38–INVARIANT_42 enforced in `verify_governance_chain.py`. **PASS 10C:** Remote CI adjudication for INVARIANT_43–50: `verification/ci-remote-evidence/record.json` + `ci_provenance` align to GitHub run id and artifact `artifact_commit_hash`; per-invariant IDs appear in uploaded `*-governance.json` `invariants_verified` (stdout is summary-only).

- **PASS 11 invariant lattice** — `00-constitution/invariant-registry.json` (domain, failure_mode, severity, enforcement_hook, evidence_hook, escalation per INVARIANT_01–50); `verification/invariant-failure-map.json`; `verification/invariant-risk-model.json`; `03-enforcement/enforcement-map.json` (`invariant_risk_bindings`). Completeness enforced in `scripts/verify_governance_chain.py`.
