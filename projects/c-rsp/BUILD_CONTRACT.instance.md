...
- **Authoritative Truth Surface:**
  ...
  - `STATUS.json`
- **Instance Artifact Path:** `projects/c-rsp/BUILD_CONTRACT.instance.md`
...
- **Required Inputs:** `projects/c-rsp/BUILD_CONTRACT.md`; `projects/c-rsp/BUILD_CONTRACT.instance.md`; FDE Draft artifact set under `docs/fde-control-plane/`, `governance-rules/`, `schemas/`, `evidence/fde-control-plane/`; repo root `STATUS.json`; explicit standalone twin repo path when available.
...
- **Rollback Procedure:** Stop on first failed check; do not promote; restore prior `BUILD_CONTRACT.instance.md`, `governance-template.lock.json`, `NEXT_CRSP_BUILD.json`; preserve failed reports; write `failure-audit.json` and `rollback-report.json`.
...
This framework is executable only through this instantiated document: `projects/c-rsp/BUILD_CONTRACT.instance.md`. Status remains **Draft** until all promotion guards and evidence for Active status are satisfied.
